...
Android TV app for NVIDIA Shield that browses Synology SMB3 shares and plays files in VLC, with Trakt metadata and TMDB fanart cards styled like the Home Assistant *UPCOMING TV* rows. Also includes **Live TV (IPTV)** with M3U playlists and XMLTV EPG, plus optional **YouTube** (TV account feed + Innertube playback).
...
4. **Notices go through `AppNoticeBus`.** All user-facing status, errors, successes, and in-progress updates (loading feeds, resolving streams, indexing, saves, auth messages, etc.) **must** use `AppNoticeBus` (`show` / `success` / `error` / `info` / `progress`, or `ForwardFlashNotice` for one-shot flashes). Hosted by `AppNoticeHost` in `MainActivity`. Do **not** add screen-local loading rows, red/green inline status `Text`, or ad-hoc toast overlays for the same job. Use `AppNoticeKind.Progress` for ongoing work and dismiss it when finished; timed kinds auto-clear.

If a change touches playback lifecycle, wake locks, Radio/Music composables, any modal/overlay, or user-visible status/loading/error UI, re-read this section and verify these rules still hold.
...
- **Search:** Channels + programme titles; measured quality badges (streamed channels only); recent searches.
...
- **Browse:** Home hotspot + left rail (toggle under Display). Opens on **all shows · recent** episode feed from the last catalog snapshot (instant), then quietly refreshes feeds in the background; Shows picker can filter to one subscription (loads that show’s full feed, listed **8 at a time** — scroll/focus the last row to load the next 8). Sort **A–Z / Recent / Genre / In progress**; episode sort **Newest / Oldest / Unplayed**; resume progress. Manual **Refresh** still force-updates with a progress overlay.
...
Ad-free via **Innertube** (Piped search/stream fallback) into ExoPlayer. Search or paste URLs; **YouTube TV account** subscription feed (device code link, like SmartTube); continue watching (last 20); related while playing.
...
| **Sleep timer** | 15→30→45→60→90→Off (click) or custom minutes (hold OK); fades then stops **all** players; tries HA standby when enabled; shared **blackout** overlay with the Black button if power-off fails |
...
| **Hard rules** | Playing keeps screen on; quit stops all in-app audio (including Podcasts); notices use `AppNoticeBus` |
...
- **Piped** — YouTube search / stream fallback
...
- Settings backup: **Settings → Backup** selects a private NAS folder and exports/imports `PallasVideoPlayer-settings.json`. The portable backup contains NAS credentials, Trakt/TMDB keys and tokens, IPTV playlists, YouTube TV link tokens + search API URL, custom radio stations, podcast OPML path + subscriptions, favorites, custom channel order/names, manual EPG assignments, EPG AI API key/provider, measured stream badges, parental settings, Display (lite visuals), home landing tile visibility, the **LAN remote control token**, and **Philips Hue** Music sync (bridge IP, username, selected lights). Import replaces those values so another TV/phone has the same setup; if this TV already has a Device id (`lounge` / `bedroom`), that id is kept so HA handoff stays correct. Caches and viewing/search history are rebuilt locally. The JSON contains secrets in readable form, so store it only in a private NAS folder. File transfer uses SMB3 on port 445 automatically; when the app is configured for HTTP browsing, this is a temporary in-memory connection and the saved HTTP mode/port are never changed.
...
- **Settings → YouTube:** **Link YouTube** shows a TV activation code (open `youtube.com/activate` / Google device page on your phone). Optional Piped API URL for search / stream fallback. Included in settings backup
...
- **Search:** channels + EPG programme titles — channel results show measured quality badges (when previously streamed) and favorite state; **OK** plays fullscreen, **hold OK / Menu** adds/removes favorite. The 10 most recent searches are saved per playlist and shown when the query is empty.
- **Quality badges:** while a channel plays, resolution / fps / HDR are measured from the decoder and cached per channel. Guide, search, history, and HUDs show badges **only** after a channel has been streamed and measured — M3U name tokens (`FHD`, `50fps`, etc.) are never shown as badges. While zapping, the HUD keeps previously measured badges until the new live measurement is confirmed (~1s of rendered frames). Fullscreen also shows the selected audio layout (`Mono`, `Stereo`, `5.1`, `7.1`, `Dolby Atmos`, etc.) in an orange badge, plus a `♫ N` indicator when multiple audio tracks are available. Hold **OK** or press **Menu** in fullscreen for detailed stream information: video codec, resolution, frame rate, bitrate, audio codec (of the selected track, e.g. AAC / Dolby Digital / MP2), layout, sample rate, language, support, and per-track details.
...
- **Sleep timer** (timer icon): shared across **NAS browser**, **Live TV**, **Radio**, **Music**, **Podcasts**, and **YouTube** — click cycles **15 → 30 → 45 → 60 → 90 → Off**; **hold OK / Menu** opens a custom minutes dialog; countdown stays visible when you switch screens; volume fades over the last minute then **stops every in-app player** (and external VLC for NAS). Optional **Home Assistant standby**: Settings → Integrations → **Sleep timer → HA standby** — POSTs to `…/api/webhook/pallas_sleep` with `{"device":"<device id>","action":"standby","blackout":true|false}` (device id from Settings → Integrations; webhook URL derived from the now-playing webhook). After stop, the app goes to a full **blackout** screen; if HA standby is off, misconfigured, or the webhook fails, that blackout remains so the panel stays dark even when the TV cannot power off.
...
- **Sleep timer** works while music is playing (same shared timer; hold OK for custom minutes)
...
- **Sleep timer** (timer icon in the Radio top bar / left rail): same shared timer — countdown continues if you started it elsewhere; fades then stops radio (and every other player)
...
3. When the sleep timer expires, Pallas stops **all** playback, enters app blackout, then POSTs `{"device":"<device id>","action":"standby","blackout":true}` when **Sleep timer → HA standby** is enabled. If that setting is off, the webhook/device id is missing, or the POST fails, blackout stays up (OK / Back wakes the UI).
